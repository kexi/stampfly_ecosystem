...
- `unity/` は 4 つ目のシミュレータ（ブラウザで動く Unity 版）を作る場所で、計画は `docs/plans/unity-simulator.md`。
  ...
  家具編集では一時停止して部屋を上から表示し、終了時に機体を出発点へ戻す。家具付きの部屋を選ぶ画面も公開済み。
  現在の実装は障害物15種・同梱10空間。追加のボーリング部屋には12 g・高さ18 cm・直径5 cmの動的なPhysXピンを10本置き、
  `RESTART`／Bで機体とピンを初期配置へ戻す。保存は初期配置を保持する。検証・公開の状況は `docs/plans/unity-simulator.md` §4に記録する。
