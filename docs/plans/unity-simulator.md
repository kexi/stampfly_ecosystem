...
状態: **実装中**。作成 2026-09-20、最終更新 2026-09-22。
...
| 操縦入力 | キーボードとタッチを選択できる。タッチは送信機既定のMode 2で、左がヨー・スロットル、右がロール・ピッチ。`KeyboardRc` が両入力のARMパルス・高度維持スイッチを共有し、`TouchRcState` が左右の軸を持つ。`TouchStick` が指ごとの捕捉、`SimHud` が中断・停止・再起動時の解除を行う。振れ幅と12 bit変換は既存と同じ。ゲームパッドは次段階 | 別のARM状態や制御則をUIに作らず、ファームウェアと既存の入力経路を共用する。スマホ実機での操作感・速度確認は別途必要 |
| スマホの表示と公開情報 | UI Toolkitで画面上のスティックと操作ボタンを表示。WebGLはCSSピクセルと描画ピクセルを揃え、切欠きを避ける。リポジトリ・Fork元・MIT全文をアイコンだけのHTMLリンクから開く。ビルドは元の `LICENSE` を `LICENSE.txt` へ原文のままコピーし、失敗時はビルド失敗 | 2本の指の操作をページのスクロールに渡さず、リンクはUnityの読込状態に依存させない。著作権表示と許諾文を配布物に残す |
...
| 追跡カメラの寄せ具合 | **距離と高さに同じ倍率を掛け、視野角（FOV、55°）は変えない。** 既定は後方 0.28 m・上方 0.128 m（比 0.55/1.2 はズームを足す前の見下ろす角）、範囲は 0.15〜3.0 m、1 段は 1.15 倍（対数の刻み）。操作はマウスホイール（前で寄る）・F ／ G・C（既定へ戻す）と、端末からの `camera.zoom` ／ `camera.state`。倍率の算術は Unity の型を持たない `ChaseZoom`（`Runtime/Ui/`）に置き、`FollowCamera` は `Time.unscaledDeltaTime` でそこへ寄っていく（一時停止中も寄せられる） | 視野角で寄せると遠近感が平らになり、機体と壁の間の距離がその距離らしく見えなくなる。目で飛ばす人はそこから接近の速さを判断するので、寄せるたびに変わるレンズはそれを奪う。既定の 1.2 m では 1280 幅の画面に機体が幅 50 px ほどしか映らず、STL 13 個へ移植した白いフレーム・脚・プロペラを細部まで見られなかったのが発端である。大きさは**軸方向の幅 0.0816 m**（衝突箱の footprint）で数える。よく挙がる 0.12 m は対角のロータ間で、それを基に決めた最初の案（0.45 m）では幅 130 px にしかならず脚が見分けられなかった。ブラウザで実測して 0.28 m（幅 200 px）に改めた。刻みを一定のメートル数にしないのは、0.15 m での 0.1 m が機体までのほとんどの距離になり、3.0 m では見て分からないからである |
...
`WorldEditSession` は変更案を検査してから `WorldLoader` で生成し直す。室外への配置、出発点の離陸空間を塞ぐ配置、500個を超える配置は拒否し、失敗した操作では場面と履歴を変えない。最大20操作を元に戻せる。外部から空間を読み直した場合は以前の履歴を破棄する。家具同士の重なりは禁止しない。
...
| Stick input | Select keyboard or touch. Touch uses the transmitter's default Mode 2: left yaw/throttle, right roll/pitch. `KeyboardRc` shares ARM pulses and the altitude-mode switch; `TouchRcState` holds the axes. `TouchStick` owns individual pointers; `SimHud` releases gestures on interruption, pause and restart. Deflection and 12-bit conversion are unchanged. Gamepads remain a later stage | Reuse the firmware and input path instead of adding UI-owned ARM state or control laws. Phone handling and performance still require device checks |
| Mobile layout and project information | UI Toolkit displays sticks and controls. WebGL uses one rendering pixel per CSS pixel and respects safe areas. Icon-only HTML links open the repository, upstream and MIT text. Builds copy the original `LICENSE` verbatim to `LICENSE.txt` and fail if copying fails | Two-finger gestures must not scroll the page. Links remain independent of Unity loading, and the distribution retains the copyright and permission notice |
...
| Chase-camera zoom | **The distance and the height take the same multiplier; the field of view (55°) never changes.** The default is 0.28 m behind and 0.128 m above (the 0.55/1.2 ratio is the look-down angle from before zoom existed), the range is 0.15–3.0 m, and one step is a factor of 1.15 (a logarithmic step). The controls are the mouse wheel (forward is closer), F / G, and C (back to the default), plus `camera.zoom` / `camera.state` from a terminal. The arithmetic lives in `ChaseZoom` (`Runtime/Ui/`), which holds no Unity type, and `FollowCamera` closes on it over `Time.unscaledDeltaTime` so zooming works while paused | Zooming with the field of view flattens perspective, so the gap between the vehicle and a wall stops looking like the distance it is — and somebody flying by eye judges closing speed from exactly that. The reason for the change: at the previous 1.2 m the airframe was only about 50 px wide on a 1280-wide picture, too small to make out the white frame, the legs and the propellers of the 13-STL port. The size is counted on the ACROSS-AN-AXIS width of 0.0816 m (the collision box's footprint); the 0.12 m usually quoted is the diagonal between opposite rotors, and a first attempt sized on it (0.45 m) gave only 130 px, at which the legs could not be told apart. Measured in the browser and corrected to 0.28 m, which gives 200 px. The step is not a fixed number of metres because 0.1 m at 0.15 m would be most of the way to the vehicle and invisible at 3.0 m |
...
`WorldEditSession` validates a proposed change before rebuilding through `WorldLoader`. It refuses placement outside the room, obstruction of the spawn's takeoff space and layouts with more than 500 items. Refused operations leave the scene and history unchanged. Up to 20 edits can be undone; external world loads invalidate the old history. Furniture overlap is allowed.
