...
既存の SILS・VPython 版・Genesis 版は**そのまま残す**。Unity 版はそれらを置き換えない。変更で既存の動作が壊れていないかを自動で確かめる試験（再確認試験、`sf sils regression` の 33 シナリオ）と、モデル一致の合否判定は、引き続き既存 SILS が担う。Unity 版で 33 シナリオの合否を再現することは目標にしない。
...
| 項目 | 内容 |
|---|---|
| 位置づけ | 既存の 3 つを残して併設する。Unity 版で 33 シナリオの合否を再現することは目標にしない |
...
| 2 ネイティブコア | `scheduler_step.cpp`、`plant.hpp` の囲み、`plant_external.cpp`、`actuator_model.hpp`、`frames_unity.hpp`、`simulator/unity/native/bridge/`（`sfu_api.h`、ログのリングバッファ）、`sfu_bridge_smoke`（Unity なしで C++ 内の簡易剛体積分により ARM → 離陸 → ホバリング） | `sf sils regression` が 33 件とも変更前と同じ結果で、ログの sha256 が一致する。`actuator_parity_test`・`frames_unity_test` が通る。最小動作確認が 2 回走って出力が一致する |
...
1. **既存を壊していないこと**: 変更前後で `sf sils build` → `sf sils regression` を実行し、33 件の結果とログの sha256 が一致することを確かめる
...
SILS, the VPython version, and the Genesis version are **all kept**. The Unity version replaces none of them. The re-verification test suite (`sf sils regression`, 33 scenarios) and the model-match pass/fail check remain the job of the existing SILS. Reproducing the 33 scenarios' verdicts in the Unity version is not a goal.
...
| Item | Content |
|---|---|
| Position | Added alongside the existing three, which are kept. Reproducing the 33 scenarios' verdicts is not a goal |
...
1. **Nothing existing is broken**: run `sf sils build` → `sf sils regression` before and after the change, and confirm that all 33 results and the log sha256 values match
