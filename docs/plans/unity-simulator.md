...
段階 1 の結果を数値付きでここに記録し、その上で段階 2 以降へ入る。**(a)〜(e) を 2026-09-20 に実施した**。(a)(c)(d)(e) は合格。(b) は呼び出しの仕組みが成立した一方で、飛行結果がメモリの配置で分かれる不具合が見つかり、Unity 経由の飛行は修正待ち。(f) は実機コントローラを要するため未実施。成果物と詳しい数値・再現手順は [`../../simulator/unity/native/README.md`](../../simulator/unity/native/README.md) にある。
