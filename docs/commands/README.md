...
| `sf pilot` | Jev による自動操縦の判断層（`bench`=往復時間の実測、`replay`=飛行ログの再生判断） | `sf pilot --help` |
...
| `sf pilot` | Jev-assisted autopilot judging layer (`bench` measures round-trip time, `replay` judges a flight log) | `sf pilot --help` |
