...
| `sf pilot bench/replay/run/say/mission/explore` | Jev による自動操縦の判断層（`docs/plans/jev-autopilot.md`）。bench=往復時間の実測、replay=飛行ログの再生判断、run=飛ばして監視（`--sils` で SILS、`--scene nominal/battery_drop/drift/wall_ahead/dead_end`。`--real` で**実機**: 飛行前点検→離陸→ホバリング→監視→着陸。`--real --preflight-only` は点検だけで離陸しない。実機は確認事項 5 項目への `y` が必須で `--yes` でも省略できず、非対話では実行しない）、say=自然言語の指示を手順に変えて飛ぶ（`--sils` 必須。`--scene` 対応、`--dry-run` は変換だけ、`--eval` は期待表との照合）、mission=経路（区間の列）を飛び、区間の境目ごとに次の一手を Jev に問う（`--sils` 必須。同梱経路は `line`／`square`／`explore_pocket`。`--dry-run` は経路の確認だけ）、explore=その場で回って 8 方位の距離を測り、開けている方位のうち目的に近いものを選ぶ（`--sils` 必須。既定の場面は `dead_end`。**既定で無効** — 現状の SILS では機体がヨー回転で落下するため、起動前に理由を表示して終了コード 1 で拒否する。解除手順は `docs/plans/jev-autopilot.md` §4.11.2）。**実機を飛ばせるのは `run --real` だけで、`say`／`mission`／`explore` は `--real` を拒否する**（移動を伴う飛行が実機で未確認のため。`docs/plans/jev-autopilot.md` §4.12）。いずれもキー不要の `--fake` あり。`wall_ahead`／`dead_end` は前方 ToF の模擬を使う場面で、前方距離が**停止距離**（固定値ではなく接近速度から算出する。`安全余裕 + 惰走係数 × 接近速度`）を下回ると Jev を待たずに `stop` する。併せて前方距離に応じて 1 回の前進量を刻み、停止しても近づき続ける場合は `back` で離れる（`docs/plans/jev-autopilot.md` 4.9 / **4.9.10**）。run/say/mission は `--web`（`--port`／`--no-browser`）で飛行中の機体と判断の流れをブラウザ表示（127.0.0.1 のみ。3D は `sf telemetry --web` と共有）。SILS 飛行は毎回 `simulator/sils/viz/out_<種別>/<日時>/` にフライトログ一式を残し、`sf sils video -m <種別>/<日時>` で動画にできる |
