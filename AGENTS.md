...
This file provides guidance to AI coding agents (Claude Code, Codex, and others) when working with code in this repository. `CLAUDE.md` only imports this file (`@AGENTS.md`); write all instructions here.
...
### 言葉遣い（用語辞書）
日本語の言い換え・使わない語・訳語・表記・文体の**基準は `docs/contributing/terminology.md`**（2026-09-13 制定。兄弟リポジトリの辞書を統合したもの）。チャット報告・文書・コミットメッセージを出す前に、同文書の「提出前チェックリスト」を適用する。ユーザーから語の指摘を受けたら、その場で同文書に追記してコミットする（指摘 1 回で以後の全セッションに効く状態を保つ）。用語の**意味**の基準は `docs/guides/glossary.md`。
...
#### 7. 計画文書の状態表記とアーカイブ禁止（Plan Status / No Archives）

2026-09-12 制定。矛盾した文書（古い計画が現行の記述として読まれる状態）の併存を防ぐため。

- `docs/plans/` の計画文書は冒頭に **状態** を明記する: `状態: 計画中 / 実装中 / 実装済み / 見直し中`（作成日・最終更新日を添える）
- **アーカイブディレクトリは作らない**（`archive/` 等）。完了・廃止した計画文書、使われなくなったスクリプト・試作コードは main から削除する。履歴は Git が保持する
- 削除するときは、削除直前のコミットに注釈付きタグ `archive/YYYY-MM-DD` を付ける。取り出しは `git show archive/YYYY-MM-DD:<path>`
- 生きている文書から削除済みの文書に触れる必要があるときは、リンクではなく「タグ名とパス」を書く（例: `docs/plans/archive/hils-firmware.md`、タグ `archive/2026-09-12`）
...
| Unity 版シミュレータの自動の飛行確認（`sf unity check fly`） | 標準出力に判定の JSON（ファイルには書かない。`> flight.json` で残す）。飛行そのものの行は上の `logs/unity/<run_id>.jsonl` に入る | JSON。`pass`・`checks[]`（各検査の `measured` と `wanted`）・`altitude`・`pacing`・`transitions[]`（状態遷移と `sim_us`）・`samples[]`・`commands[]`（発行した `cmd_id`）。実装は `tools/unity_check/fly_check.py` | `jq -r '.checks[] \| select(.pass\|not)' flight.json`、`sf unity logs --cmd $(jq -r '.commands[]\|select(.command=="rc.arm")\|.cmd_id' flight.json \| head -1)` |
| Unity 版シミュレータをエディタで再生したとき（`RemoteBridge` が場面にあるとき） | `simulator/unity/Logs/stampfly/<run_id>.jsonl`（git 管理外）。同時に Unity のコンソールにも同じ行が出る | JSON Lines。ページと同じ形式・同じ鍵（`sf unity serve` は関わらないので `logs/unity/` には入らない）。実装は `simulator/unity/Assets/StampFly/Runtime/Remote/EditorFileLogSink.cs` | `jq -c 'select(.src=="fw")' simulator/unity/Logs/stampfly/<run_id>.jsonl`、`ls -t simulator/unity/Logs/stampfly/ \| head -1` |
| Unity 版シミュレータを公開ページ（GitHub Pages）で開いたとき | どこにも書かない。ページの中に直近 2000 行を保持するだけ（`/api/hello` に応えるサーバが無いため） | JSON Lines。ローカル配信のときと同じ形式で、`run_id` はページが発行する | ブラウザの devtools で `window.stampfly.logs()`（文字列で返る）、`window.stampfly.download()`（`<run_id>.jsonl` として保存） |
| Unity 版ネイティブコアの最小動作確認（`sfu_bridge_smoke`／`sfu_external_smoke`／`sfu_module_check.mjs`。`--log-jsonl <path>` を渡したときだけ） | 渡したパス。`just unity-native-test` は `simulator/unity/native/build-native/logs/<名前>.jsonl`・`build-wasm/logs/<名前>.jsonl` に書く（ビルドディレクトリの中なので git 管理外） | JSON Lines。ファームの `ESP_LOGx` を**文字列にする前の形**で受けた行（`src: "fw"`・`event: "fw.log"`・`tag`・`sim_us`）と、橋渡し自身の行（`src: "bridge"`・`bridge.boot`／`bridge.shutdown`／`bridge.error`）。形式は下の「新しく書くコードのログの決まり」、実装は `simulator/unity/native/bridge/sfu_log_jsonl.hpp` | `jq -c 'select(.level=="warn")' <path>`、`jq -r 'select(.tag=="StateManager") \| "\(.sim_us) \(.msg)"' <path>` |
