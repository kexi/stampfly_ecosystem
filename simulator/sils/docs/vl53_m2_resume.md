...
実装場所: **`simulator/sils/devices/vl53_device.cpp`**（virtual_board.cpp から分離。probe と共有）の
`sils_vl53::xfer` / `fill_histogram` / `target_mm` と定数 `ZERO_DIST_PHASE` / `AMBIENT_BIN_STRIP` / `REG_MODE_START`。
virtual_board.cpp の `sils_board_i2c_xfer` は 0x29/0x30 を `sils_vl53::set_distance_mm`+`xfer` へ委譲。
...
`simulator/sils/smoke/vl53_probe.cpp`（CMake `SILS_BUILD_VL53_PROBE`, OFF 既定）。無改変 ST ドライバを
...
```bash
cmake -S simulator/sils -B simulator/sils/build -DSILS_BUILD_VL53_PROBE=ON
cmake --build simulator/sils/build --target vl53_probe -j
./simulator/sils/build/vl53_probe 700 6   # target_mm frames
```
...
- **アドレス**: bottom は power-on 0x29 → reg `0x0001` に 0x30 を書いて再アドレス → 以後 0x30。
  front は 0x31（未モデル → graceful 無効化、放置可）。`sils_board_i2c_xfer` は 0x29 と 0x30 両方を `vl53_xfer` へ。
...
cmake --build simulator/sils/build --target emu_vehicle -j

# 距離掃引（現状は全部 status=255。M2 完了で ~target が status=0 で出るべき）
for mm in 100 289 500 1000 1500; do
  out=$(SILS_VL53_TEST_MM=$mm ./simulator/sils/build/emu_vehicle \
        simulator/sils/models/stampfly.xml 22000000 2>&1 | grep "bottom=" | tail -1)
  echo "target=${mm} -> $out"
done
...
1. `SILS_VL53_TEST_MM=R` で ToFTask が `bottom≈R status=0`（2 フレーム目以降）。誤差は対称ピークで ±96mm、
   肩 skew 較正後は数 mm。
2. `SILS_VL53_TEST_MM` を外して `vl53_target_mm()` が `g_plant->tof()×1000` を返す本番経路で、
   Plant 距離追従を確認（craft を非接地高度に置く手段が要る — テスト override か、後述の ALT_HOLD）。
3. 既存動作は維持: 全 SILS ビルド緑 / emu_vehicle_old・emu_vehicle 決定論 / hover_espnow(14)・console_cli(8) PASS。
...
> **→ 検証済みの完全な手順は `simulator/sils/docs/hover_resume.md` を正とする**（実コード裏取り済み）。
...
## 8. 関連
- メモリ: `project_stampfly_emulator.md`（全体経緯）, `project_sils_reset.md`（方針）, `feedback_checklist_discipline.md`。
