...
/**
 * @file plant.cpp
 * @brief SILS physics plant implementation (motor model + truth + synthetic sensors).
 *        SILS 物理プラント実装（モータモデル＋真値＋合成センサ）。
 *
 * Sign/formula provenance: simulator/sils/docs/coordinate_frames.md §4 and the
 * firmware ESKF observation models. All frame conversions go through frames.hpp.
 *
 * 符号・式の出典: simulator/sils/docs/coordinate_frames.md §4 とファーム ESKF の
...
// takes no dt). If the caller invokes step() at an irregular rate (the SILS
...
// モデル timestep を1回進めるため、不規則な呼び出し（SILS スケジューラの仮想時計ジャンプ）
...
void Plant::substep(float h)
{
    ...
    // thrust_efficiency (which scales the mean too, and was found — 2026-08-02 SILS
    ...
    // clean path stays byte-identical — enable via SILS_EMU_TORQUE_AUTHORITY
    // (sf sils scenario --torque-authority).
    ...
    // ―― 2026-08-02 の SILS 注入実験で、平均も縮小する thrust_efficiency では10-20%の
    ...
    // SILS_EMU_TORQUE_AUTHORITY（sf sils scenario --torque-authority）で有効化。
    //
