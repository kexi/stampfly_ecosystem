# ESKF accel-attitude χ² 判定（カイ二乗判定）の過剰棄却 — 調査ノート

最終更新: 2026-06-08（✅ §8 で根治。真因＝過剰棄却）

> **用語と読み方**：この現象を当初「**ラッチアップ**」（電子回路 latch-up の借用＝固着）と呼び、
> ファイル名にも残っているが、計測の結果「推定が固着して回復不能」ではなく「**χ² 判定が加速度
...
- 加速度計は重力＋運動加速度（specific force）を測る。マニューバ中は運動加速度が大きく、`accel_comp`
  （フロー由来 α-β トラッカで運動加速度を予測・減算）でも残差が残る → **常時 ~66% 棄却**
  （PASS ケースでも！ 姿勢はかろうじて静穏時の補正で保たれる綱渡り）。
- 判定は**ハードカットオフ**（`return`）。一度 推定姿勢が大きくズレると、**正しい加速度測定が
  「推定から見て外れ値」に見え**、判定がその補正を棄却 → 推定はズレたまま → 次の innovation は
  さらに大きい → 永久に棄却（**latch-up**）。判定が「直すための補正」自身を弾く。

### 計測した証拠（pos_yaw, live 3.696 FAIL ケース）

```
chi2: d2=897.3 gate=7.8 reject=65%      ← d² が桁違いに大きくなる（推定が遠い）
...
SILS に 1S LiPo 電池電圧低下モデルを実装し、actuator が実電圧（sensor_power）を live 追従。
→ 端子電圧が動的に動くと、INA3221 の 8mV 量子化が**一定バイアスでなく微小ディザ**になり、
ラッチ閾値を一定方向に押し続けないため作動しない。**χ² 判定の構造は変えていない**ので、
別の外乱が臨界に当たれば再発しうる。

## 5. 根治の候補（このタスクでやること）

**AGENTS.md 原則: 制御/推定器パラメータの変更は必ず SILS 数値シミュレーションで裏付けてから。**
定性推測で決めない。以下を SILS で掃引・比較し、最良策を数値選定する。

1. **χ² 判定しきい値の緩和**: `accel_chi2_gate` 7.8（95%点）→ 11.3（99%点）や更に上。
   - 期待: ラッチ閾値が上がり、臨界マニューバでも棄却され過ぎず回復余地が残る。
   - リスク: 緩めすぎると真の外れ値（運動加速度による加速度計測定値の汚染）を通し姿勢が「水平」と誤推定。
     `disturb`/N1/N2 で姿勢精度が劣化しないかを同時に見る。
2. **ラッチ回復ロジック**: 連続 N 回棄却が続いたら「推定が発散した」と判断し、判定を一時的に
   無効化する / 適応 R を一気に膨らませて測定を取り込み直す（外れ値判定が回復を妨げない設計）。
3. **適応 R の強化**: 既存の `R = R_base²·(1 + k·|‖a‖−g|²)`（マニューバ中は重み下げ）を、
   固定しきい値の判定の代わりに主役にする（ソフトに下げて棄却ゼロ化）。`updateAccelAttitude` の
   adaptive R コメント参照。
...
- **臨界の消失を直接確認**: actuator を fixed 3.7→3.696（推力+0.1%）にしても pos_yaw が
  発散しないこと（`SILS_EMU_BATTERY=off` で電池電圧低下を切り、量子化バイアスを再現できる）。
...
- **掃引で数値選定**: 判定しきい値（7.8/11.3/16/…）× シナリオの表を作り、最良を選ぶ。
...
source setup_env.sh
sf sils build
# 量子化バイアスで臨界を再現（電池電圧低下OFF＝固定電圧に量子化を載せる）:
...
| 場所 | 役割 |
|------|------|
| `components/sf_estimator_eskf/eskf_core.cpp::vectorUpdate3` | χ² 判定本体（d²>7.8 で return） |
...
| `simulator/sils/emu/emu_main.cpp` | `SILS_EMU_BATTERY=off` で電池電圧低下切替 |
| `simulator/sils/plant/plant.cpp` | 電池電圧低下モデル（当座の回避） |
...
- **判定しきい値の緩和（7.81→11.3/16/25/50/100）**: 非単調（11.3/16 は PASS、25/50/100 は FAIL）。
  臨界を動かすだけで根治でない（高すぎると運動加速度を通し姿勢が崩れる）。
- **ノルム判別（‖a‖≈g なら回復のため判定を無効化する）**: 逆効果（band=2.0 で att_rmse=202 に爆発）。
  ヨー主体のマニューバでは汚染時もノルムが≈g に留まり判別不能。

### 教訓
定性推測（「数百度ラッチ」「ノルムで判別」）は2回とも外れ、**計測（d²/innov/R/est を時系列ログ）
が真因（タイトな R による過剰棄却）を一度で示した**。AGENTS.md 原則どおり数値で裏付けて選定。
