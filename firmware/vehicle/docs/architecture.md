...
| 2 | 状態推定 | センサデータから姿勢/位置/速度を推定（差し替え可能） | Algorithm |
...
| 6 | 制御 | セットポイント追従演算（差し替え可能） | Algorithm |
...
- **インターフェース統一**: 制御・状態推定は差し替え可能な統一インターフェース
...
横断ルール R が「コンポーネント間の構造規約」であるのに対し、INV は **個々の制御則・状態機械が常に満たすべき設計契約**である。**新機能の追加や要件変更で、ある機能の前提が変わったときは、ここに列挙された INV と、それを参照する `@design` タグを必ず再照合し、古い前提を持つ既存コンポーネントを洗い出して直すこと（リップル確認）。** SILS がたまたま通っても INV 違反は既存動作の破壊とみなす。違反・矛盾を見つけたら設計矛盾として即時報告する。
...
設計上の14責務は、ESP-IDF のコンポーネント単位に展開すると、以下の対応で実装される。1つの責務がインターフェース層 + 実装層に分かれる場合（差し替え可能設計のため）や、責務に直接対応しない基盤コンポーネント（Pub-Subフレームワーク、数学ライブラリ）が存在する。
...
| 2 | 状態推定 | `sf_estimator`（インターフェース）, `sf_estimator_eskf`（ESKF実装） | 差し替え可能設計のため2層 |
| 3 | 状態管理 | `sf_state` | — |
| 4 | フェイルセーフ | `sf_failsafe` | — |
| 5 | 離着陸マネージャー | `sf_takeoff_landing` | — |
| 6 | 制御 | `sf_controller`（インターフェース）, `sf_controller_pid`（PID実装）, `sf_app_hooks`（L1 差し替え口: `sf::app::controller()` / `estimator()` / `start()` と既定実装） | 差し替え可能設計のため2層＋差し替え口 |
...
| 不変条件 | 照合結果 |
|---------|---------|
| INV-1（単一の姿勢＋レートパイプライン） | フックは `IController` **全体**を差し替える。`compute()` の呼び出し位置・回数は変わらず、並列の姿勢則は生じない。**自作コントローラは鉛直フェーズ（Grounded / TakeoffClimb / Airborne / Landing）の扱いを自身の `compute()` 内で引き継ぐ責務を負う**（`onTakeoff()` / `onLanding()` 等の通知は従来どおり届く） |
...
**現状（2026-09-09時点）:** `firmware/vehicle` の `sf_actuator::mixerCompute()` は物理単位（推力[N]・トルク[Nm]）の B⁻¹ 幾何配分＋非線形モータ曲線（`thrustToDuty`: モータ曲線 + バッテリ電圧補償）を実装するが、`firmware/workshop` の `ws_internal.hpp::resolve()` は `vehicle_old` から移植した単純な線形近似（`duty = thrust + k*(±R±P±Y)`、`k=0.25/3.7` の経験定数1個に幾何・モータ曲線・電圧補償を丸め込む）を**独自に複製**している。両者は R12（HAL共有）の精神に反して乖離しており、`sf sysid fit` 等の同定ツールがどちらのミキサーで飛んだログかを事前に判別できないと正しく逆算できない、という不具合を引き起こした（2026-09-08〜09 の一連の修正、`--mixer {legacy,vehicle}` として一時的に吸収）。

**提案する設計（次回作業で着手、着手前に SILS 再確認試験（変更で既存の動作が壊れていないかを自動で確かめる試験）・既存実習ゲインへの影響をシミュレーションで検証すること — AGENTS.md「制御系パラメータ変更」原則）:**
...
2. **サンプル周期オーダーのタイミング精度が要求され、かつ数値で実証されている**（状態遷移経由の遅延では推定が壊れることを SILS の判定等で示せる。実証なき特例は認めない）
3. **対象が推定器の内部連続状態** である（積分器・トラッカ・フィルタ履歴。controller / actuator の状態は対象外）

**クラスB に課す制約（越権防止）:**

- **estimation 層に閉じる**: 自身が所有する推定器のみを触る。他コンポーネント（controller / actuator）の reset をしてはならない
