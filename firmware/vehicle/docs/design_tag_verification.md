...
- **判断**: コードを要件§9 の規定値（3.0G／800dps／3.4V＋連続2回）に合わせるか、要件§9 を実装値（4.0G／1000dps／3.3V）に合わせて改訂するか。**安全に直結するためユーザー判断とする。** 閾値変更は実機の挙動・誤発報率に影響するため、変更時は AGENTS.md 原則に従い根拠を添える。
